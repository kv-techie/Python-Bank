   ...
   Salary Credit Day: {self.salary_day} of each month{last_salary_info}"""
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "lastSalaryDate": self.last_salary_date.isoformat() if self.last_salary_date else None
        }
...
class SalaryProfileFactory:
    ...
    @staticmethod
    def create(gross_salary: float, salary_day: int) -> Tuple[bool, str, Optional[SalaryProfile]]:
        """
    ...
    def show_salary_brackets():
        ...
        print("-" * 70)
        print(f"{'Tax Threshold':<25} {'₹1,50,000/month (₹18,00,000/year)':>30} {'15% above':<15}")
        print("=" * 70)
    ...
    def show_salary_breakdown(gross_salary: float):
        ...
        Args:
            gross_salary: Monthly gross salary
        """
        gross, tax, net, annual_net = SalaryProfileFactory.calculate_take_home(gross_salary)
        ...
        if tax > 0:
            ...
            print("-" * 60)
            print(f"{'Net Salary (Take Home)':<30} ₹{net:>10,.2f} ₹{annual_net:>13,.2f}")
        else:
