...
class Account:
    ...
    def __init__(self, customer_id: str, username: str, password: str, first_name: str,
                 last_name: str, dob: str, gender: str, account_type: str, account_number: str,
                 balance: float = 0.0, transactions: Optional[List[Transaction]] = None,
                 failed_attempts: int = 0, locked: bool = False, pending_amb_fees: float = 0.0):
    ...
    def get_today_withdrawals(self) -> float:
        ...
        for txn in self.transactions:
            if txn.type == "WITHDRAW":
                try:
                    txn_date = datetime.strptime(txn.timestamp, "%d-%m-%Y %H:%M:%S").date()
                    if txn_date == today:
                        total += txn.amount
                except:
                    pass
    ...
    def _check_and_apply_amb_fee(self):
        ...
        if self.balance < amb_req and self.balance >= self._amb_fee_amount:
            ...
            print(f"AMB fee of Rs. {self._amb_fee_amount} INR charged (balance below Rs. {amb_req} INR).")
            
        elif self.balance < amb_req and self.balance < self._amb_fee_amount:
            self.pending_amb_fees += self._amb_fee_amount
            print(f"AMB fee of Rs. {self._amb_fee_amount} INR accrued. Will be collected on next deposit.")
    ...
    def deposit(self, amount: float):
        ...
        if amount > self._max_deposit_per_txn:
            print(f"Maximum allowed per deposit is Rs. {self._max_deposit_per_txn:.2f} INR. Please split large deposits.")
            return
    ...
    def withdraw(self, amount: float):
        ...
        if self.is_minor_account:
            ...
            if today_withdrawals >= self._minor_daily_withdrawal_limit:
                print(f"Minor account daily withdrawal limit reached (Rs. {self._minor_daily_withdrawal_limit:.2f} INR per day).")
            ...
            if today_withdrawals + amount > self._minor_daily_withdrawal_limit:
                ...
                print(f"Withdrawal amount exceeds daily limit for minor accounts.")
                print(f"Daily withdrawal limit: Rs. {self._minor_daily_withdrawal_limit:.2f} INR")
            ...
            if today_transactions + amount > self._minor_daily_transaction_limit:
                ...
                print(f"Withdrawal amount exceeds daily transaction limit for minor accounts.")
                print(f"Daily transaction limit: Rs. {self._minor_daily_transaction_limit:.2f} INR")
                print(f"Already transacted today: Rs. {today_transactions:.2f} INR")
                print(f"Remaining limit: Rs. {remaining:.2f} INR")
                return
        
        if self.balance - amount < self._min_operational_balance:
            print(f"Insufficient funds. You must keep at least Rs. {self._min_operational_balance:.2f} INR.")
            return
        ...
        if self.is_minor_account:
            ...
            print(f"Remaining daily withdrawal limit: Rs. {self._minor_daily_withdrawal_limit - new_withdrawals:.2f} INR")
            print(f"Remaining daily transaction limit: Rs. {self._minor_daily_transaction_limit - new_transactions:.2f} INR")
    ...
    def transfer(self, recipient: 'Account', amount: float, requested_mode: str):
        ...
        if self.is_minor_account:
            ...
            if today_transactions + amount > self._minor_daily_transaction_limit:
                remaining = self._minor_daily_transaction_limit - today_transactions
                print(f"Transfer amount exceeds daily transaction limit for minor accounts.")
                print(f"Daily transaction limit: Rs. {self._minor_daily_transaction_limit:.2f} INR")
                print(f"Already transacted today: Rs. {today_transactions:.2f} INR")
                print(f"Remaining limit: Rs. {remaining:.2f} INR")
                return
        ...
        if requested_mode != "INTER_ACCOUNT":
            if amount >= 200000.0 and requested_mode == "NEFT":
                actual_mode = "RTGS"
                print(f"Amount Rs. {amount:.2f} INR is >= Rs. 2,00,000.00. Automatically routing to RTGS.")
            elif amount < 200000.0 and requested_mode == "RTGS":
                actual_mode = "NEFT"
                print(f"Amount Rs. {amount:.2f} INR is below RTGS threshold (Rs. 2,00,000.00). Routing to NEFT instead.")
        
        enforce_min = actual_mode != "INTER_ACCOUNT"
        if enforce_min and (self.balance - amount < self._min_operational_balance):
            print(f"Insufficient funds. Must keep at least Rs. {self._min_operational_balance:.2f} INR.")
            return
        
        self.balance -= amount
        recipient.balance += amount
        
        cheque_id = f"CHQ{random.randint(1000000000, 9999999999)}" if actual_mode != "INTER_ACCOUNT" else None
        ...
        print(f"{actual_mode} transfer successful. Sent: Rs. {amount:.2f} INR | Transaction ID: {txn_send.id}")
        if cheque_id:
    ...
    def show_transactions(self):
        ...
        if not self.transactions:
            ...
            for t in self.transactions:
                print(f"{t.id:<15} {t.timestamp:<20} {t.type:<10} Rs. {t.amount:<10.2f} Rs. {t.resulting_balance:<10.2f} INR")
            ...
            if self.is_minor_account:
                ...
                print(f"Withdrawal Limit: Rs. {self._minor_daily_withdrawal_limit:.2f} INR (Used: Rs. {today_withdrawals:.2f}, Remaining: Rs. {self._minor_daily_withdrawal_limit - today_withdrawals:.2f} INR)")
                print(f"Transaction Limit: Rs. {self._minor_daily_transaction_limit:.2f} INR (Used: Rs. {today_transactions:.2f}, Remaining: Rs. {self._minor_daily_transaction_limit - today_transactions:.2f} INR)")
    ...
    def add_recurring_bill(self, bill: RecurringBill):
        ...
        print(f"Recurring bill added: {bill.name} - Rs. {bill.base_amount:.2f} INR ({bill.frequency})")
    ...
    def show_recurring_bills(self):
        ...
        if not self.recurring_bills:
            ...
            print("\nRecurring Bills")
            print("=" * 70)
            print(f"{'ID':<10} {'Name':<20} {'Amount':<15} {'Frequency':<12} {'Due Day':<8}")
            print("=" * 70)
            for bill in self.recurring_bills:
                print(f"{bill.id:<10} {bill.name:<20} Rs. {bill.base_amount:<14.2f} {bill.frequency:<12} {bill.day_of_month:<8}")
            print("=" * 70)
    ...
    def set_salary(self, gross_salary: float, salary_day: int):
        ...
        self.salary_profile = SalaryProfile(gross_salary=gross_salary, salary_day=salary_day)
        ...
        if tax > 0:
            ...
            print(f"   Net Monthly Salary: Rs. {net_salary:,.2f} INR (No tax - income below Rs. 18,00,000.00/year)")
    ...
    def from_storage(customer_id: str, username: str, password: str, first_name: str,
                     last_name: str, dob: str, gender: str, account_type: str, account_number: str,
                     balance: float, transactions: List[Transaction], failed_attempts: int,
                     locked: bool, pending_amb_fees: float) -> 'Account':
...
Branch Code: {Account.ACCOUNT_NUMBER_PREFIX}"""
    ...
    @staticmethod
    def create_account(customer_id: str, username: str, password: str, first_name: str,
                      last_name: str, dob: str, gender: str, account_type: str,
                      account_number: Optional[str] = None) -> 'Account':
    ...
    def from_dict(data: dict) -> 'Account':
        ...
        acc.recurring_bills = [RecurringBill.from_dict(b) for b in data.get("recurringBills", [])]
