    ...
    def verify_document(self, doc_index: int) -> tuple[bool, str]:
        ...
        try:
            if 0 <= doc_index < len(self.documents):
                ...
                return True, f"✅ Document verified"
            else:
            ...
            "documents": [
                {
                    ...
                    "verificationDate": doc.verification_date.isoformat() if doc.verification_date else None
                }
        ...
        for doc_data in data.get("documents", []):
            doc = DeductionDocument(
                ...
                verification_date=datetime.fromisoformat(doc_data["verificationDate"]) 
                    if doc_data.get("verificationDate") else None
            )
...
class TaxProfile:
    ...
    def get_total_eligible_deductions(self) -> float:
        ...
        return sum(e.eligible_amount for e in self.exemptions if e.status != DeductionStatus.REJECTED)
