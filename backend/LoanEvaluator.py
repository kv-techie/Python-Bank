...
class LoanEvaluator:
    ...
    def evaluate(
        ...
        if not cibil_score or cibil_score < 700:
            return False, f"CIBIL score too low ({cibil_score}). Minimum required: 700", details
        ...
        if dti_ratio > 0.5:
            return False, f"High debt-to-income ratio ({dti_ratio * 100:.1f}% > 50%)", details
        ...
        max_loan_multiplier = LoanEvaluator.LOAN_TO_INCOME_RATIOS.get(employer_category, 5.0)
        ...
        for score_threshold in sorted(LoanEvaluator.CIBIL_RATE_ADJUSTMENTS.keys(), reverse=True):
            if cibil_score >= score_threshold:
                adjusted_rate = interest_rate + LoanEvaluator.CIBIL_RATE_ADJUSTMENTS[score_threshold]
                break
        ...
        if cibil_score >= 800:
            return True, f"Excellent credit profile - Loan approved at premium rate ({adjusted_rate:.2f}%)", details
        elif cibil_score >= 750:
            return True, f"Good credit profile - Loan approved ({adjusted_rate:.2f}%)", details
        elif cibil_score >= 700:
            return True, f"Fair credit profile - Loan approved with processing fee ({adjusted_rate:.2f}%)", details
        else:
    ...
    @staticmethod
    def calculate_suggested_interest_rate(cibil_score: int, base_rate: float = 9.5) -> float:
        """
