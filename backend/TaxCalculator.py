...
class TaxCalculator:
    ...
    @staticmethod
    def calculate_tax_without_deductions(gross_annual_salary: float) -> Tuple[float, float]:
        """
    ...
    def calculate_monthly_tax_with_deductions(
        ...
        annual_deductions: Dict[str, float]
    ) -> Tuple[float, float, float]:
        """
        ...
        taxable_income, annual_tax, tax_rate = TaxCalculator.calculate_tax_with_deductions(
            gross_annual,
            annual_deductions
        )
    ...
    def generate_tax_report(
        ...
        taxable_income, tax_with_deductions, tax_rate = TaxCalculator.calculate_tax_with_deductions(
            gross_annual_salary,
            deductions
        )
        
        # Calculate tax without deductions for comparison
        tax_without_deductions, _ = TaxCalculator.calculate_tax_without_deductions(gross_annual_salary)
        ...
        savings_percentage = (annual_tax_saved / tax_without_deductions * 100) if tax_without_deductions > 0 else 0
...
"""
    ...
    def compare_regimes(
        gross_annual_salary: float,
        deductions_old_regime: Dict[str, float]
    ) -> Tuple[str, Dict]:
        """
        ...
        old_regime_taxable, old_regime_tax, old_regime_rate = TaxCalculator.calculate_tax_with_deductions(
            gross_annual_salary,
            deductions_old_regime
        )
        
        # New regime (no deductions, no exemptions)
        new_regime_tax, new_regime_rate = TaxCalculator.calculate_tax_without_deductions(
            gross_annual_salary
        )
